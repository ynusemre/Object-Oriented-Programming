...
namespace Abstract_Class{
	...
	HexArray1D::HexArray1D(const HexArray1D &object) :AbstractHex(object){     //Copy Constructor.
		hexCells=(Cell*)calloc((this->number*this->number),sizeof(Cell));
		
		hexCells=object.hexCells;
	}
	//Copy Assignment Operator.
	HexArray1D& HexArray1D::operator=(const HexArray1D &object){
	...
	HexArray1D::~HexArray1D(){    //Destructor for the HexArray1D class
		if(hold5!=0){
			if(ctr4==0){
				delete []arr5;
				delete []arr6;
	...
	bool HexArray1D::isEnd(const int &r)const{
		int counter10=0,counter11=0,counter12=0,counter13=0;
		
		if(r==0){
			for(int i=0;i<number;i++){
				for(int j=(i*number);j<number*(i+1);j++){
					if(hexCells[j].getCell()==1 && hexCells[j].getRow()==1){
						counter10++;
						break;
					}}}
			for(int i=0;i<number;i++){
				for(int j=i;j<number*number;j+=number){
					if(hexCells[j].getCell()==2 && hexCells[j].getRow()==2){
						counter11++;
						break;
					}}}
			if(counter10==number){
	...
	inline void HexArray1D::control(const int &i)const{
		int counter=0,counter2=0;
		CellState cell1=dot;
		CellState cell2=cross;
		CellState cell3=circle;

		if(hexCells[i].getCell()==cell2 && hexCells[i].getRow()==cell2){
			cout<<" X ";
			counter++;
		}
		if(hexCells[i].getCell()==cell3 && hexCells[i].getRow()==cell3){
			cout<<" O ";
			counter2++;
		}
		if(hexCells[i].getCell()==cell1 && hexCells[i].getRow()==cell1 && counter==0 && counter2==0){
			cout<<" . ";
		}}
	...
	void HexArray1D::writeFromFile(string file_name,char arr[7],int arr5[100],int arr6[100],int arr7[100],int arr8[100],int &number,int f,int l,int wch_user,int who_vs_who){
		ofstream file;
		string str;
		file.open(file_name);
		...
		for(int i=0;i<number;i++){
			...
			for(int j=(i*number);j<number*(i+1);j++){
				...
				if(hexCells[j].getCell()==cell2 && hexCells[j].getRow()==cell2){
					file<<" X ";
					counter++;
				}
				if(hexCells[j].getCell()==cell3 && hexCells[j].getRow()==cell3){
					file<<" O ";
					counter2++;
				}
				if(hexCells[j].getCell()==cell1 && hexCells[j].getRow()==cell1 && counter==0 && counter2==0){
					file<<" . ";
				}}
	...
	HexArray1D::Cell HexArray1D::operator()(const int &x,const int &y)const{
		HexArray1D::Cell cells;
		cells.setRow(x);
		cells.setCell(y);
		...
		int index=((x-1)*number)+y;
		int counter=0;
		
		
		if(hexCells[index].getCell()==cell2 && hexCells[index].getRow()==cell2){
			counter++;
		}	
			
		
		if(hexCells[index].getCell()==cell3 && hexCells[index].getRow()==cell3){
			counter++;
		}	
		if(counter==0){
	...
	//Function to Reset game content.
	void HexArray1D::reset(){
		hexCells=(Cell*)calloc((number*number),sizeof(Cell));
	...
	/*Requests the game board from the user again and gives a warning message if an invalid login occurs.*/
	...
	HexArray1D::Cell HexArray1D::lastMove()const{
		HexArray1D::Cell cells;
		cells.setRow(h1);
		cells.setCell(h2);
		int ctr6=0;
	...
	*/				
	HexArray1D::Cell HexArray1D::play()const{
		int counter8=0;
	...
	bool HexArray1D::operator==(const HexArray1D &object)const{
		...
		for(int i=0;i<number;i++){
			for(int j=(i*number);j<number*(i+1);j++){
				if(hexCells[j].getCell()!=object.hexCells[j].getCell() && hexCells[j].getRow()!=object.hexCells[j].getRow()){
					counter++;
					break;
				}}}
		if(counter!=0){
	...
	*/		
	...
	void HexArray1D::play(int &number2,int num3){
				...
				while(flag5!=0){
					...
					if(num1[0]=='L' && num1[1]=='O' && num1[2]=='A' && num1[3]=='D'){
						...
						if(counter==0){
							...
							int count18=HexArray1D::numberofMoves(f,l);
							cout<<endl;
	...
	///////////////*If the user has entered a valid coordinate it goes into this if.*////////////////			
				if(counter4==1 && counter3==1 && counter5+counter6==0 && counter13==0){
					h1=num3;
					h2=number2;
					int index=((num3-1)*number)+number2;
					if(nbr==0){
						if(ctr3==0){
							HexArray1D :: Cell cells;
							cells.setRow(num3);
							...
							if(counter17%2==0){
								...
								f++;
								hexCells[index].setter(1);
							}
							if(counter17%2==1){
								arr7[l]=cells.getRow();
								arr8[l]=cells.getCell();
								l++;
								hexCells[index].setter(2);
							}
							counter17++;
							count18=HexArray1D::numberofMoves(f,l);
							hold4=count18;
						...
						if(ctr3==1){
							...
							hexCells[index].setter(1);
							count18=HexArray1D::numberofMoves(f,f-1);
							hold4=count18;
							cout<<endl;
							cout<<"The number of marked(non-empty) cells in all the games:"<<count18<<endl;
							int *arr18=score();
					...
					if(nbr==1 || nbr==2){
						if(num5==0){	
							for(int i=0;i<f;i++){
								...
								int index2=((x)*number)+d;
								hexCells[index2].setter(1);
							}
							for(int i=0;i<l;i++){
								z=arr7[i]-1;
								k=arr8[i];
								
								int index2=((z)*number)+k;
								hexCells[index2].setter(2);
							}
							if(ctr3==0){
								HexArray1D :: Cell cells2;
								cells2.setRow(num3);
					...
					if(x3==true){
						if(ctr3==0){
							...
							if(counter17%2==0){	
								...
								counter17=0;
								if(hold3==1){
									HexArray1D::hold2=HexArray1D::hold4;
									if(hold1==hold2){
										number2=18;
									}
								}	
								if(hold3==0){
									hold1=HexArray1D::hold4;
									hold4=0;
									hold3++;
								}
								hold5++;
