...
namespace Abstract_Class{
	...
	HexVector::HexVector(const HexVector &object) :AbstractHex(object){		//Copy Constructor.
	...
	HexVector& HexVector::operator=(const HexVector &object){
	...
	HexVector::~HexVector(){     //Destructor for the HexVector class.
		if(hold5!=0){
			if(ctr4==0){
				delete []arr5;
				delete []arr6;
	...
	bool HexVector::operator==(const HexVector &object)const{
	...
	bool HexVector::isEnd(const int &r)const{
	...
	inline void HexVector::control(const int &i,const int &j)const{
	...
	void HexVector::writeFromFile(string file_name,char arr[7],int arr5[100],int arr6[100],int arr7[100],int arr8[100],int &number,int f,int l,int wch_user,int who_vs_who){
		ofstream file;
		string str;
		file.open(file_name);
	...
	HexVector::Cell HexVector::lastMove()const{
		HexVector::Cell cells;
		cells.setRow(h1);
		cells.setCell(h2);
		int ctr6=0;
	...
	HexVector::Cell HexVector::operator()(const int &x,const int &y)const{
		HexVector::Cell cells;
		cells.setRow(x);
		cells.setCell(y);
	...
	*/				
	HexVector::Cell HexVector::play()const{
		int counter8=0;
	...
	*/			
	...
	void HexVector::play(int &number2,int num3){
				...
				while(flag5!=0){
					...
					if(num1[0]=='L' && num1[1]=='O' && num1[2]=='A' && num1[3]=='D'){
						...
						if(counter==0){
							...
							int count18=HexVector::numberofMoves(f,l);
							cout<<endl;
				...
				if(counter4==1 && counter3==1 && counter5+counter6==0 && counter13==0){
					...
					if(nbr==0){
						if(ctr3==0){
							...
							counter17++;
							count18=HexVector::numberofMoves(f,l);
							hold4=count18;
						...
						if(ctr3==1){
							...
							hexCells[num3-1][number2].setter(1);
							count18=HexVector::numberofMoves(f,f-1);
							hold4=count18;
							cout<<endl;
							cout<<"The number of marked(non-empty) cells in all the games:"<<count18<<endl;
							int *arr18=score();
					...
					if(x3==true){
						if(ctr3==0){
							...
							if(counter17%2==0){	
								...
								counter17=0;
								if(hold3==1){
									HexVector::hold2=HexVector::hold4;
									if(hold1==hold2){
										number2=18;
									}
								}	
								if(hold3==0){
									hold1=HexVector::hold4;
									hold4=0;
									hold3++;
								}
								hold5++;
